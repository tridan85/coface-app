...
function makeEmailAgente(r) {
  ...
  const cfg = getClientEmailCfg(r, "agente");
  const to  = cfg.to || getAgentEmail(r) || "";
  const cc  = cfg.cc;
  const bcc = cfg.bcc;
...
function makeEmailAzienda(r) {
  ...
  const to  = r?.email || "";
  const agentEmail = getAgentEmail(r);
  const cc  = cfg.includeAgentInCc
    ? [agentEmail, ...cfg.cc].filter(Boolean)
    : cfg.cc;
  const bcc = cfg.bcc;
...
function makeEmailAnnullo(r) {
  ...
  const cfg = getClientEmailCfg(r, "annullo");
